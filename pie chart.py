...
rated_movies = rated_movies.drop(rated_movies.index[[0]])
from pylab import *
rating_count = rated_movies[["movie_id","rating"]].groupby("rating").count()
suma = rating_count.values.sum()
fracs = rating_count.values*100/suma
# make a square figure and axes
figure(1, figsize=(6,6))
ax = axes([0.1, 0.1, 0.8, 0.8])

# The slices will be ordered and plotted counter-clockwise.
labels = rating_count.index.tolist()
fracs = fracs.tolist()
explode=tuple([0]*rating_count.values.size)
...
title('Rating Porcent', bbox={'facecolor':'0.8', 'pad':5})
